...
func TestBoilerplate(t *testing.T) {
	...
	for _, useCase := range [][]string{
		{"already has header", "*.go", "main_test.go", goTestWithHeader, "false", "noError"},
		{"report", "*.go", "main.go", goFile, "true", "Error"},
	} {
		t.Run(useCase[0], func(t *testing.T) {
			...
			err := boilerplate(*logger, osf, []string{useCase[1]}, nil, header, convBool(useCase[4]))
			if useCase[5] == "noError" {
				assert.NoError(err)
			} else {
				assert.Error(err)
			}
